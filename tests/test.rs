...
use calamine::{
    ...
    ExcelDateTimeType, FreezePane, HeaderRow, HorizontalAlignment, IconSetType, Ods, Range, Reader,
    ReaderRef, Sheet, SheetType, SheetVisible, TimePeriodType, UnderlineStyle, VerticalAlignment,
...
#[test]
fn test_xlsx_freeze_panes() {
    let mut excel: Xlsx<_> = wb("freeze_panes.xlsx");

    // Two frozen rows.
    assert_eq!(
        excel.worksheet_freeze_panes("FrozenRows").unwrap(),
        Some(FreezePane {
            frozen_cols: 0,
            frozen_rows: 2,
            top_left_cell: Some((2, 0)),
        })
    );

    // One frozen column.
    assert_eq!(
        excel.worksheet_freeze_panes("FrozenCols").unwrap(),
        Some(FreezePane {
            frozen_cols: 1,
            frozen_rows: 0,
            top_left_cell: Some((0, 1)),
        })
    );

    // Frozen rows and columns.
    assert_eq!(
        excel.worksheet_freeze_panes("FrozenBoth").unwrap(),
        Some(FreezePane {
            frozen_cols: 1,
            frozen_rows: 2,
            top_left_cell: Some((2, 1)),
        })
    );

    // No pane element at all.
    assert_eq!(excel.worksheet_freeze_panes("NoFreeze").unwrap(), None);

    // A split (non-frozen) pane is not reported as a freeze pane.
    assert_eq!(excel.worksheet_freeze_panes("SplitOnly").unwrap(), None);

    // Unknown sheet name is an error.
    assert!(excel.worksheet_freeze_panes("NoSuchSheet").is_err());

    // Index based lookup.
    assert_eq!(
        excel.worksheet_freeze_panes_at(2).unwrap().unwrap(),
        Some(FreezePane {
            frozen_cols: 1,
            frozen_rows: 2,
            top_left_cell: Some((2, 1)),
    ...
    assert!(excel.worksheet_freeze_panes_at(99).is_none());
}

#[test]
fn test_xls_freeze_panes() {
    let excel: Xls<_> = wb("freeze_panes.xls");

    // Two frozen rows.
    assert_eq!(
        excel.worksheet_freeze_panes("FrozenRows"),
        Some(FreezePane {
            frozen_cols: 0,
            frozen_rows: 2,
            top_left_cell: Some((2, 0)),
    ...
    // One frozen column.
    assert_eq!(
        excel.worksheet_freeze_panes("FrozenCols"),
        Some(FreezePane {
            frozen_cols: 1,
            frozen_rows: 0,
            top_left_cell: Some((0, 1)),
    ...
    // Frozen rows and columns.
    assert_eq!(
        excel.worksheet_freeze_panes("FrozenBoth"),
        Some(FreezePane {
            frozen_cols: 1,
            frozen_rows: 2,
            top_left_cell: Some((2, 1)),
    ...
    // No pane record at all.
    assert_eq!(excel.worksheet_freeze_panes("NoFreeze"), None);

    // A split (non-frozen) pane is not reported as a freeze pane.
    assert_eq!(excel.worksheet_freeze_panes("SplitOnly"), None);

    // Unknown sheet name.
    assert_eq!(excel.worksheet_freeze_panes("NoSuchSheet"), None);

    // Index based lookup.
    assert_eq!(
        excel.worksheet_freeze_panes_at(2),
        Some(FreezePane {
            frozen_cols: 1,
            frozen_rows: 2,
            top_left_cell: Some((2, 1)),
    ...
    assert_eq!(excel.worksheet_freeze_panes_at(99), None);
}

#[test]
fn test_xlsb_freeze_panes() {
    let mut excel: Xlsb<_> = wb("freeze_panes.xlsb");

    // Frozen pane: 1 column and 2 rows, top-left cell B3.
    assert_eq!(
        excel.worksheet_freeze_panes("Visible").unwrap(),
        Some(FreezePane {
            frozen_cols: 1,
            frozen_rows: 2,
            top_left_cell: Some((2, 1)),
    ...
    // A split (non-frozen) pane is not reported as a freeze pane.
    assert_eq!(excel.worksheet_freeze_panes("Hidden").unwrap(), None);

    // No pane record at all.
    assert_eq!(excel.worksheet_freeze_panes("VeryHidden").unwrap(), None);
    ...
    // Index based lookup.
    assert_eq!(
        excel.worksheet_freeze_panes_at(0).unwrap().unwrap(),
        Some(FreezePane {
            frozen_cols: 1,
            frozen_rows: 2,
            top_left_cell: Some((2, 1)),
...
#[test]
fn test_xlsx_freeze_panes_real_world_file() {
    // File generated by a third party tool with a frozen pane:
    // <pane ySplit="4" topLeftCell="A29" activePane="bottomLeft" state="frozen"/>
    let mut excel: Xlsx<_> = wb("issue_530.xlsx");

    assert_eq!(
        excel.worksheet_freeze_panes("出勤分类组织汇总表").unwrap(),
        Some(FreezePane {
            frozen_cols: 0,
            frozen_rows: 4,
            top_left_cell: Some((28, 0)),
        })
    );
}
