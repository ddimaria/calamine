    ...
    loop {
        ...
        match xml.read_event_into(&mut buf) {
            Ok(Event::Start(ref e)) => {
                ...
                if local.as_ref() == "clrScheme" {
                    in_clr_scheme = true;
                } else if in_clr_scheme {
                    if clr_elements.iter().any(|&el| local.as_ref() == el) {
                        current_element = Some(local.as_ref().to_string());
            ...
            Ok(Event::End(ref e)) => {
                ...
                if current_element
                    .as_ref()
                    .is_some_and(|el| el.as_str() == local.as_ref())
                {
                    current_element = None;
                }
            }
...
fn parse_theme_srgb_or_sys(attributes: quick_xml::events::attributes::Attributes) -> Option<Color> {
    ...
    for attr in attributes.flatten() {
        match attr.key.as_ref() {
            "lastClr" => {
                if let Some(c) = parse_hex_attr(attr.value.as_bytes()) {
                    return Some(c);
                }
            }
            "val" if color.is_none() => {
                color = parse_hex_attr(attr.value.as_bytes());
            }
            _ => {}
        }
    }
...
pub(crate) fn parse_color(
    ...
    for attr in attributes {
        match attr.key.as_ref() {
            ...
            "theme" => {
                if let Ok(theme_value) = attr.value.parse::<u8>() {
                    color = Some(get_theme_color(theme_value, theme_colors));
                }
            }
            "indexed" => {
                if let Ok(indexed_value) = attr.value.parse::<u8>() {
                    color = Some(get_indexed_color(indexed_value));
                }
            }
            "tint" => {
                tint = attr.value.parse::<f64>().ok();
            }
            _ => {}
        }
    }
...
pub fn parse_font<RS: BufRead>(
    ...
    loop {
        ...
        match xml.read_event_into(&mut buf) {
            Ok(ref event @ (Event::Start(ref e) | Event::Empty(ref e))) => {
                ...
                match e.local_name().as_ref() {
                    "name" => {
                        ...
                        if !is_empty {
                            if name.is_none() {
                                name = read_string(xml, QName("name"))?;
                            } else {
                    ...
                    "sz" => {
                        ...
                        for attr in e.attributes() {
                            let attr = attr?;
                            if attr.key.as_ref() == "val" {
                                size_str = Some(attr.value.to_string());
                                break;
                            }
                        }
                        if !is_empty {
                            if size_str.is_none() {
                                size_str = read_string(xml, QName("sz"))?;
                            } else {
                                xml.read_to_end_into(e.name(), &mut Vec::new())?;
                            }
                        }
                    ...
                    "family" => {
                        if !is_empty {
                            if let Some(family) = read_string(xml, QName("family"))? {
            ...
            Ok(Event::End(ref e)) if e.local_name().as_ref() == "font" => break,
        ...
        match xml.read_event_into(&mut buf) {
            Ok(Event::Start(ref e) | Event::Empty(ref e)) => match e.local_name().as_ref() {
                "patternFill" => {
                    for attr in e.attributes() {
                        let attr = attr?;
                        if attr.key.as_ref() == "patternType" {
                            let pattern_str: &str = &attr.value;
            ...
            Ok(Event::End(ref e)) if e.local_name().as_ref() == "fill" => break,
...
pub fn parse_border<RS: BufRead>(
    ...
    for attr in start_elem.attributes() {
        let attr = attr?;
        match attr.key.as_ref() {
            "diagonalDown" => has_diagonal_down = &*attr.value != "0",
            "diagonalUp" => has_diagonal_up = &*attr.value != "0",
            _ => {}
        }
    }
    ...
    loop {
        ...
        match xml.read_event_into(&mut buf) {
            Ok(ref event @ (Event::Start(ref e) | Event::Empty(ref e))) => {
                ...
                match e.local_name().as_ref() {
                    "left" | "right" | "top" | "bottom" | "diagonal" => {
                        ...
                        for attr in e.attributes() {
                            let attr = attr?;
                            if attr.key.as_ref() == "style" {
                                let style_str: &str = &attr.value;
                            ...
                            loop {
                                ...
                                match xml.read_event_into(&mut inner_buf) {
                                    Ok(Event::Start(ref inner_e) | Event::Empty(ref inner_e))
                                        if inner_e.local_name().as_ref() == "color" =>
                                    {
            ...
            Ok(Event::End(ref e)) if e.local_name().as_ref() == "border" => break,
...
pub fn parse_alignment<RS: BufRead>(
    ...
    for attr in start_elem.attributes() {
        ...
        match attr.key.as_ref() {
            ...
            "indent" => {
                if let Ok(indent) = attr.value.parse::<u8>() {
                    alignment = alignment.with_indent(indent);
                }
            }
            "shrinkToFit" => {
                let shrink_str: &str = &attr.value;
