    ...
    RS: Read + Seek,
{
            ...
            match self.xml.read_event_into(&mut self.buf) {
                ...
                Ok(Event::Start(c_element)) if c_element.local_name().as_ref() == "c" => {
                    ...
                    let style_id = style_attr
                        .and_then(|s| atoi_simd::parse::<usize, true, false>(s.as_bytes()).ok())
                        .unwrap_or(0);
            ...
            match self.xml.read_event_into(&mut self.buf) {
                ...
                Ok(Event::Start(c_element)) if c_element.local_name().as_ref() == "c" => {
                    ...
                    let style_id = c_element
                        .raw_attr("s")?
                        .map(|v| atoi_simd::parse::<usize, true, false>(v.as_bytes()).unwrap_or(0))
                        .unwrap_or(0);
    ...
    pub fn next_style(&mut self) -> Result<Option<Cell<Style>>, XlsxError> {
        loop {
            self.buf.clear();
            match self.xml.read_event_into(&mut self.buf) {
                Ok(Event::Start(ref row_element)) if row_element.local_name().as_ref() == "row" => {
                    let attribute = row_element.raw_attr("r")?;
                    if let Some(range) = attribute {
                        let row = get_row(range)?;
                        self.row_index = row;
                    }
                }
                Ok(Event::End(ref row_element)) if row_element.local_name().as_ref() == "row" => {
                    self.row_index += 1;
                    self.col_index = 0;
                }
                Ok(Event::Start(ref c_element)) if c_element.local_name().as_ref() == "c" => {
                    let attribute = c_element.raw_attr("r")?;
                    ...
                    let style = if let Ok(Some(style_id_str)) = c_element.raw_attr("s") {
                        if let Ok(style_id) =
                            atoi_simd::parse::<usize, true, false>(style_id_str.as_bytes())
                        {
                            if style_id < self.styles.len() {
                                let mut s = self.styles[style_id].clone();
                                s.style_id = Some(style_id as u32);
                    ...
                    loop {
                        self.cell_buf.clear();
                        match self.xml.read_event_into(&mut self.cell_buf) {
                            Ok(Event::End(ref e)) if e.local_name().as_ref() == "c" => break,
                            Ok(Event::Eof) => return Err(XlsxError::XmlEof("c")),
                            Err(e) => return Err(XlsxError::Xml(e)),
                            _ => (),
                        }
                ...
                Ok(Event::End(ref e)) if e.local_name().as_ref() == "sheetData" => {
                    return Ok(None);
                }
                Ok(Event::Eof) => return Err(XlsxError::XmlEof("sheetData")),
                Err(e) => return Err(XlsxError::Xml(e)),
    ...
    pub fn next_style_id(&mut self) -> Result<Option<(u32, u32, usize)>, XlsxError> {
        loop {
            ...
            match self.xml.read_event_into(&mut self.buf) {
                Ok(Event::Start(ref row_element)) if row_element.local_name().as_ref() == "row" => {
                    if let Some(row_index) = row_element.raw_attr("r")? {
                        self.row_index = atoi_simd::parse::<u32, true, false>(row_index.as_bytes())
                            .unwrap_or(1)
                            .saturating_sub(1);
                    }
                }
                ...
                Ok(Event::Start(ref c_element)) if c_element.local_name().as_ref() == "c" => {
                    ...
                    let style_id = if let Ok(Some(style_id_str)) = c_element.raw_attr("s") {
                        atoi_simd::parse::<usize, true, false>(style_id_str.as_bytes()).unwrap_or(0)
                    } else {
                        0
                    };
